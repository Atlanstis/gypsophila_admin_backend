...
export class CreateV1021706255916886 implements MigrationInterface {
  name = 'CreateV1021706255916886';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`psn_profile_game\` (\`create_time\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`update_time\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`id\` varchar(36) NOT NULL, \`is_favor\` tinyint NOT NULL COMMENT '是否收藏' DEFAULT false, \`sync_time\` timestamp NOT NULL COMMENT '同步时间' DEFAULT CURRENT_TIMESTAMP, \`psn_profile_id\` int NULL, \`psn_game_id\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
