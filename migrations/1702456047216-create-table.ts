...
export class CrateTable1702456047216 implements MigrationInterface {
  name = 'CrateTable1702456047216';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`permission\` ADD CONSTRAINT \`FK_b4083bde507bb8b760a2aaf9c08\` FOREIGN KEY (\`menu_id\`) REFERENCES \`menu\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE \`role_menu_permission\` ADD CONSTRAINT \`FK_636978ae05637a0d5318e836c6a\` FOREIGN KEY (\`role_id\`) REFERENCES \`role\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE \`role_menu_permission\` ADD CONSTRAINT \`FK_5b7e1e9365298e5f9da0d30b781\` FOREIGN KEY (\`menu_id\`) REFERENCES \`menu\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE \`role_menu_permission\` ADD CONSTRAINT \`FK_5b71be11007b819e4a3dad6d6c1\` FOREIGN KEY (\`permission_id\`) REFERENCES \`permission\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
