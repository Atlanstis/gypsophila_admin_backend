...
export class CreateV1031710397356368 implements MigrationInterface {
  name = 'CreateV1031710397356368';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`mhxy_channel\` (\`id\` int NOT NULL AUTO_INCREMENT, \`key\` varchar(16) NULL COMMENT '途径 key，唯一，用于业务判断', \`name\` varchar(16) NOT NULL COMMENT '名称', \`is_default\` tinyint NOT NULL COMMENT '是否默认' DEFAULT 0, \`parent_id\` int NOT NULL COMMENT '父级 id' DEFAULT '0', PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
